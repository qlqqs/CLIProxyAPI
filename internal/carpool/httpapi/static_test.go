...
func TestRegisterStaticRoutes(t *testing.T) {
	...
	if redirect.Code != http.StatusPermanentRedirect || redirect.Header().Get("Location") != "/carpool/" {
		t.Fatalf("/carpool response = %d, location %q", redirect.Code, redirect.Header().Get("Location"))
	}

	index := httptest.NewRecorder()
	engine.ServeHTTP(index, httptest.NewRequest(http.MethodGet, "/carpool/", nil))
	if index.Code != http.StatusOK || index.Header().Get("Content-Security-Policy") == "" || index.Header().Get("Cache-Control") != "no-cache" {
		t.Fatalf("/carpool/ response = %d, headers %#v", index.Code, index.Header())
	}
	...
	engine.ServeHTTP(asset, httptest.NewRequest(http.MethodGet, "/carpool/assets/app.js", nil))
	...
	request := httptest.NewRequest(http.MethodGet, "/carpool/assets/app.js", nil)
...
func TestStaticRoutesDoNotCatchUnknownCarpoolPaths(t *testing.T) {
	...
	engine := gin.New()
	RegisterStaticRoutes(engine)
	for _, path := range []string{"/carpool/missing", "/carpool/api/v1/missing", "/carpool/assets/missing.js"} {
