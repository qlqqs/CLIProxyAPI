#!/usr/bin/env bash
# Rebuild and update a local CLIProxyAPI Docker Compose installation.
...
重新编译源码并重建 CLIProxyAPI 容器，不会拉取远程 CLIProxyAPI 应用镜像。
USAGE
}
...
[[ -f "$INSTALL_DIR/docker-compose.yml" ]] || { echo "不是有效的 CLIProxyAPI 安装目录：$INSTALL_DIR" >&2; exit 1; }
